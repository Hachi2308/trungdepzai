...
export const SettingsModal: React.FC<SettingsModalProps> = ({ 
  ...
  // Check if environment supports auto-key selection
  const supportsAutoKey = typeof window !== 'undefined' && (window as any).aistudio?.openSelectKey;
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar">
          ...
          {activeTab === 'api' && (
              <div className="space-y-6">
                ...
                <div>
                   ...
                   <div className="relative">
                      ...
                      <input
                          ...
                          className="w-full bg-slate-800 border border-slate-700 rounded-xl py-3 pl-10 pr-3 text-sm text-slate-200 focus:ring-2 focus:ring-indigo-500 focus:outline-none placeholder-slate-600 font-mono"
                          placeholder="AIza..."
                      />
                   </div>
                   <p className="text-xs text-slate-500 mt-2">
                      Enter your Google Gemini API key here. It will be stored locally in your browser. 
                      <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:text-indigo-300 ml-1">
                          Get a key here.
                      </a>
                   </p>
                </div>
                ...
                {/* Google Account Connection (Conditional) */}
                {supportsAutoKey && (
                        ...
                        <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700/50">
                            <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center">
                                    <div className={`w-2 h-2 rounded-full mr-2 ${hasEnvKey ? 'bg-green-500' : 'bg-red-500'}`}></div>
                                    <span className="text-sm text-slate-400">
                                        Status: <span className={hasEnvKey ? "text-green-400 font-medium" : "text-red-400 font-medium"}>{hasEnvKey ? "Connected" : "Not Configured"}</span>
                                    </span>
                                </div>
                            </div>
                            ...
                            >
                                ...
                                {hasEnvKey ? "Update Connection" : "Connect Google Account"}
                            </button>
                        </div>
                    </div>
                )}
